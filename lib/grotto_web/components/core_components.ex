defmodule GrottoWeb.CoreComponents do
  @moduledoc """
  ...
  def card_modal(assigns) do
    ...
    <div
      ...
      class="relative z-50 hidden m-auto"
    >
      <div
        class="fixed inset-0 overflow-y-auto h-full w-full bg-black/50 m-auto"
      ...
      >
        <div class="flex h-auto items-center justify-center">
          <div class="w-full h-auto max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              id={"#{@id}-container"}
              phx-window-keydown={JS.exec("data-cancel", to: "##{@id}")}
              phx-key="escape"
              phx-click-away={JS.exec("data-cancel", to: "##{@id}")}
              class="shadow-neutral-700/10 h-full bg-neutral-100 overflow-y-visible min-h-96 ring-zinc-700/10 relative hidden rounded-lg p-4 shadow-lg ring-1 transition"
            >
  ...
  def modal(assigns) do
    ...
    >
      ...
      <div
        class="fixed inset-0 overflow-y-auto m-auto"
        aria-labelledby={"#{@id}-title"}
        aria-describedby={"#{@id}-description"}
        role="dialog"
        aria-modal="true"
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              phx-key="escape"
              phx-click-away={JS.exec("data-cancel", to: "##{@id}")}
              class="shadow-neutral-700/10 overflow-y-visible min-h-96 ring-zinc-700/10 relative hidden rounded-lg bg-white p-4 shadow-lg ring-1 transition"
            >
              <div class="absolute top-6 right-5">
                <button
                  phx-click={JS.exec("data-cancel", to: "##{@id}")}
    ...
    <div>
      ...
      <div id={"#{@action}-input-#{@id}"} class={"hidden"}>
        <form phx-submit={JS.pop_focus() |> JS.hide(to: "##{@action}-input-#{@id}") |> JS.show(to: "##{@action}-render-#{@id}")}>
          <input phx-hook="Typing" phx-blur={JS.push(@action) |> JS.hide(to: "##{@action}-input-#{@id}") |> JS.show(to: "##{@action}-render-#{@id}")} type="text" id={"#{@action}-input-field-#{@id}"} {@rest} />
        </form>
      </div>
    </div>
    """
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(GrottoWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(GrottoWeb.Gettext, "errors", msg, opts)
    end
  end
